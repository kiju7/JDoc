...
- **아카이브 직접 파싱** — ZIP/GZ/TAR/TAR.GZ 내부 문서를 디스크에 풀지 않고 메모리에서 스트리밍 변환. 멤버는 한 번에 하나만 메모리에 상주하며, 중첩 아카이브 재귀(깊이 제한)·압축폭탄 방어(해제 중 크기·비율 강제)·CP949 파일명 변환 지원. 단일 스레드로 동작하며 호출자가 문서 단위로 병렬화 가능
...
jdoc docs.zip --max-member-mb 64            # 멤버당 해제 후 크기 상한(MiB)
...
- **압축폭탄 방어** — 헤더의 크기 필드를 신뢰하지 않고 해제 도중 출력 바이트를 계수해 강제. 기본값: 멤버당 512MiB(실질 메모리 상한), 호출당 누적 64GiB·멤버 수 200,000(CPU 시간 가드 — 메모리와 무관), 압축비 1000:1, 재귀 깊이 3 (`ConvertOptions::archive`). 멤버당·압축비·깊이 초과는 해당 멤버만 스킵하고 순회를 계속하며, 누적·멤버 수 초과만 순회를 중단
