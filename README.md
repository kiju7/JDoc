...
**아카이브 (압축 해제 없이 직접 파싱):** ZIP, GZ, TAR, TAR.GZ, 7Z, ALZ, EGG, RAR(store 멤버)
...
- **아카이브 직접 파싱** — ZIP/GZ/TAR/TAR.GZ/7Z/ALZ/EGG/RAR 내부 문서를 디스크에 풀지 않고 메모리에서 스트리밍 변환. 멤버는 한 번에 하나만 상주하고, 중첩 아카이브는 재귀 처리하며, 손상·미지원 멤버는 해당 멤버만 오류로 기록하고 순회 지속
