...
- 예상 공수: RAR4 해제 엔진 이식·검증 1–2주, RAR5 포함 시 +1–2주.
  ...
  메모리 상한 설계가 주된 작업.
- 성능 실측(2026-07-20, RAR5 `-m3` 91MB→14MB, 동일 출력 확인): 공식 unrar 7.23
  ~500MB/s vs libarchive(bsdtar 3.7.4) ~105MB/s — 약 4–5배 격차이나 문서 아카이브
  워크로드에서는 해제가 아닌 파싱·변환이 지배 비용이라 실사용 영향 미미.
  계획 가정: "공식 대비 2–5배 느림, 총합 성능은 reference 우위 유지".

남은 리스크: 실제품 RAR4(WinRAR 6.x 이하 생성, 유니코드 이름·솔리드·볼륨 조합)
픽스처 부재 — 합성 픽스처와 공식 unrar 교차 검증으로 대체했다.
